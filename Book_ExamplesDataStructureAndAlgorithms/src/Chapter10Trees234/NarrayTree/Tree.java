...
public class Tree<T> {
	...
	private void getPath(Node<T> node, ArrayList<Node<T>> currentPath,
		...
		if (node.getChildren().size() == 0) {
			...
			paths.add((ArrayList<Node<T>>) currentPath.clone());
		}else{
		for (Node<T> child : node.getChildren())
	...
	public ArrayList<Node<T>> getPathsFromRootToAnyLeaf_harman() {
		//ArrayList<ArrayList<Node<T>>> paths = new ArrayList<ArrayList<Node<T>>>();
		ArrayList<Node<T>> longestPath = new ArrayList<Node<T>>();
		LinkedList<Node<T>> currentPath = new LinkedList<Node<T>>();
		getPath_haramn(root, currentPath, longestPath);

		return longestPath;
	}

	private void getPath_haramn(Node<T> node, LinkedList<Node<T>> currentPath,
			ArrayList<Node<T>> longestPath) {
		if (currentPath == null)
			return;
		...
		for (Node<T> child : node.getChildren())
			getPath_haramn(child, currentPath, longestPath);
		}
		//Node<T> noderemoved=
